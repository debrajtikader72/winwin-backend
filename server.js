...
app.post('/api/send-otp', async (req, res) => {
    res.status(200).json({ message: "OTP Simulated" });
});

// 2. Register Route (Bypasses the strict OTP email check)
app.post('/api/register', async (req, res) => {
    ...
    // Check if they entered at least some random numbers for the OTP
    if (otp.length < 4) {
        return res.status(400).json({ message: "Please enter a valid simulated OTP" });
    }
